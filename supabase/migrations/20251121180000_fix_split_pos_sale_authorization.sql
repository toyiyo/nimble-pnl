-- Fix authorization in split_pos_sale to allow SECURITY DEFINER functions to call it
-- The issue: apply_rules_to_pos_sales runs as SECURITY DEFINER, but split_pos_sale checks auth.uid()
-- Solution: Make split_pos_sale also use SECURITY DEFINER with proper checks
...
LANGUAGE plpgsql
SECURITY DEFINER -- This allows it to be called by other SECURITY DEFINER functions
...
COMMENT ON FUNCTION split_pos_sale IS 'Splits a POS sale into multiple categories. Can be called by users with owner/manager role or by SECURITY DEFINER functions.';
