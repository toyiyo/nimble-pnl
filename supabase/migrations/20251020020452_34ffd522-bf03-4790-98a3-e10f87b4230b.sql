-- Fix NULL handling in entry_number generation and improve uniqueness
-- 1. Guard against NULL stripe_transaction_id using COALESCE with transaction UUID
-- 2. Add transaction UUID to RECLASS entries for guaranteed uniqueness
-- This ensures entry_number is always meaningful, unique, and parseable
...
BEGIN
  ...
  ELSE
    ...
    ELSE
      -- Initial categorization - create new journal entry with NULL-safe entry number
