...
## Category: PDF Generation (jsPDF)

### [2026-04-26] `align: 'right'` ignores `charSpace` — measure manually
- **Mistake:** PR #479 placed the MICR line with `doc.text(text, rightX, y, { align: 'right', charSpace: 0.018 })`, expecting the rightmost character to land at `rightX`. In production the line printed past the right edge of the paper.
- **Correction:** jsPDF's `align: 'right'` derives its left offset from `getTextWidth(text)`, which measures the glyphs **without** the `charSpace` (`Tc`) operator. With `N` characters and `charSpace = s`, the rendered right edge overshoots by `(N − 1) × s`. For a 32-char MICR line and `s = 0.018"` that's ~0.56" — enough to walk off an 8.5" page. Fix: compute `leftX = rightX − (measuredTextWidth + (N − 1) × charSpace)` and emit `doc.text(text, leftX, y, { charSpace })` without the `align` option. PR #480.
- **Rule:** Whenever `charSpace` (or any `Tc`-equivalent letter spacing) is set on jsPDF text, do not rely on `align: 'right'` or `align: 'center'`. Compute the placement manually using `getTextWidth(text) + charSpace × (charCount − 1)`.

---

## Category: Domain Specs vs Production Reality

### [2026-04-26] When the spec has multiple "valid" zones, copy what real systems do
- **Mistake:** First draft of the MICR-positioning fix used 5/16" (ANSI X9 position 1) as the right margin — technically the rightmost legal position. The user pushed back: "we can't afford to get checks rejected. Also go figure out what check format Gusto and Toast use since we've been printing checks with them prior and they work well."
- **Correction:** ANSI X9.100-160-1 reserves positions 1–12 (the rightmost 1.5") for the **AMOUNT field encoded by the receiving bank** at deposit time, plus position 13 as a mandatory blank spacer. Issuer-printed characters must land at position 14 or earlier (≥ 1.9375" from the right edge). Cross-checking real production PDFs: Toast lands the on-us right edge ~3.68" from the right; Gusto left-anchors near the bottom-left. Both leave well over 1.9375" of clear space. Updated the helper to anchor at `pageWidth − 1.9375"`.
- **Rule:** For check printing, payroll output, tax forms, and other regulated artifacts, look up what major production systems (Toast, Gusto, ADP, etc.) actually do before trusting spec text alone. Specs frequently allow multiple "valid" zones; the safe choice is the zone real industry players use.

---

## Category: Font Sizing for Spec-Driven Output (TTF / PDF)

### [2026-04-26] "12pt = spec" assumes a TTF digitized for that — verify unitsPerEm
- **Mistake:** PR #480 fixed MICR positioning but kept `setFontSize(12)`, which the standard MICR-vendor docs say is the spec-compliant size ("designed to meet ANSI/ABA when printed at exactly point size 12 on a 600+ dpi laser printer"). User noticed our rendered samples looked smaller than Toast/Gusto reference checks. Investigation: the bundled `src/assets/fonts/micr-e13b.ttf` has `unitsPerEm = 4096` (atypical — most TTFs use 1000 or 2048). The "12pt = spec" claim implicitly assumes the standard unitsPerEm. With upm=4096, the rendered character height was 0.078" (67% of ANSI 0.117") and pitch was 0.083" (67% of 0.125" 8 cpi spec) — well below spec, real bank-rejection risk.
- **Correction:** Compute the spec-matching point size from the font's actual metrics. Formula: `pt = (target_inches × upm × 72) / units`. For our TTF with bbox height 1919 units and target 0.117": `pt = (0.117 × 4096 × 72) / 1919 ≈ 18`. At 18pt the same TTF hits both ANSI dimensions exactly (height 0.1171", advance 0.1250"). Bumped `setFontSize(12) → setFontSize(18)` and dropped `MICR_CHAR_SPACE_INCHES` from 0.018 to 0 (the font's own advance at 18pt now *is* 8 cpi — extra Tc would overshoot). PR #482.
- **Rule:** When a spec calls for a physical dimension (mm, inches, cpi pitch) and the implementation goes through `setFontSize(pt)`, never trust vendor docs that say "use NN pt" without verifying the TTF's `unitsPerEm` and bbox. Inspect the TTF (or use `getTextWidth`) and back-solve from the spec's required dimension. Document the chosen pt as `<NAME>_FONT_POINT_SIZE` constant with a comment that ties it to the dimension and the font's metrics, so a future font swap can re-derive it.

### [2026-04-26] Lock spec-compliance values at the byte-stream level, not just at the helper-math level
- **Observation:** PR #480 added two CRITICAL geometry tests that asserted `computeMicrPlacement` math and the rendered `Td` X coordinate were correct. Both tests still passed when the source code was at out-of-spec `setFontSize(12)` + `charSpace=0.018`, because they only checked horizontal placement, not the font size or Tc operator that determine on-paper character size. A regression that silently dropped back to 12pt would not have triggered either test.
- **Rule:** When a fix lives in two layers — a constant in the source AND its effect in a generated artifact (PDF stream, image, binary) — the test suite must include at least one assertion at the artifact layer. For jsPDF: regex the output for `Tf <size>` and `Tc <value>` ops near the relevant Tj. For PNG/SVG: parse the metadata. For raw bytes: compare a checksum of the canonical region. Helper-math tests confirm "the formula is right"; artifact-level tests confirm "the formula was actually applied with the spec values." Both are needed.
