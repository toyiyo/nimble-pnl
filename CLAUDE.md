...
### pg_cron Jobs
Never inline `net.http_post` with a hardcoded project URL in a `cron.schedule` command—preview branches and local `db reset` stacks apply every migration and would fire at prod. Schedule `SELECT public.cron_invoke_edge('<function-name>', '<body>'::jsonb)` instead: it holds the single hardcoded prod URL and no-ops off-prod (`public.is_production()` reads the `deploy_env` marker seeded at migration time from data presence, never live data). Pure-SQL cron bodies (no `net.http_post`) need no guard. If a future migration `CREATE OR REPLACE`s `cron_invoke_edge` or `cron_edge_url`, re-issue `REVOKE ALL ... FROM PUBLIC, anon, authenticated` in that same migration—`REPLACE` preserves prior grants, it does not reset them. If prod is ever rebuilt by replaying migrations onto an empty database (DR), the marker won't self-seed (no restaurants exist yet)—re-seed manually: `INSERT INTO public.deploy_env (key, value) VALUES ('environment','production')`.
