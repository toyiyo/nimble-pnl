-- Fix bulk_process_historical_sales and process_unified_inventory_deduction to use sale_date for inventory transactions

-- Update process_unified_inventory_deduction to use sale_date for created_at
...
BEGIN
    -- Convert sale_date to timestamp (using start of day in restaurant timezone would be ideal, but for now use the date)
    v_transaction_timestamp := (p_sale_date || ' 00:00:00')::timestamp with time zone;
    ...
    LOOP
        ...
        ELSIF v_purchase_unit_lower IN ('bottle', 'container', 'can', 'jar') THEN
            ...
            BEGIN
                IF v_ingredient_record.size_value IS NULL OR v_ingredient_record.size_unit IS NULL THEN
                    RAISE NOTICE 'Missing size information for product "%" (ID: %). Using 1:1 ratio.',
        ...
        -- Record inventory transaction with the ACTUAL SALE DATE
        INSERT INTO inventory_transactions (
            ...
            reference_id,
            performed_by,
            created_at  -- NOW USING v_transaction_timestamp INSTEAD OF now()
        ) VALUES (
            p_restaurant_id,
            v_ingredient_record.product_id,
            ...
            v_reference_id,
            auth.uid(),
            v_transaction_timestamp  -- FIXED: Use sale date instead of now()
        );
