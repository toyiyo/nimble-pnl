-- Fix duplicate journal entry issue in categorize_bank_transaction function
-- The problem: entry_number 'BANK-' || stripe_transaction_id can be duplicated
-- Solution: Add timestamp or check for existing entries
...
BEGIN
  ...
  IF v_is_reclassification THEN
    -- Create reclassification journal entry with unique timestamp
    INSERT INTO journal_entries (
      ...
      'RECLASS-' || TO_CHAR(now(), 'YYYYMMDD-HH24MISS-US'),
  ...
  ELSE
    ...
    ELSE
      -- Initial categorization - create new journal entry with unique entry number
      -- Using timestamp with microseconds to ensure uniqueness
      INSERT INTO journal_entries (
        restaurant_id,
        entry_date,
        entry_number,
        ...
        v_transaction.restaurant_id,
        v_transaction.transaction_date,
        'BANK-' || v_transaction.stripe_transaction_id || '-' || TO_CHAR(now(), 'YYYYMMDD-HH24MISS-US'),
