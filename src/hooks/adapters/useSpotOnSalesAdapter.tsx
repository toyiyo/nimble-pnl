...
export const useSpotOnSalesAdapter = (restaurantId: string | null): POSAdapter => {
  const { isConnected, connection } = useSpotOnIntegration(restaurantId);
  ...
  const fetchSales = useCallback(async (
    ...
    try {
      ...
      console.error('Error fetching SpotOn sales:', error);
      return [];
    }
  }, [isConnected]);
  ...
  const syncToUnified = useCallback(async (restaurantId: string): Promise<number> => {
    ...
    try {
      // Sync SpotOn data to unified_sales table
      const { data, error } = await supabase.rpc('sync_spoton_to_unified_sales' as any, {
      ...
      if (syncedCount > 0) {
        toast({
          title: "Sales synced",
          description: `${syncedCount} new sales records synced from SpotOn.`,
        });
      }

      return syncedCount;
    } catch (error: any) {
      console.error('Error syncing SpotOn sales:', error);
      toast({
