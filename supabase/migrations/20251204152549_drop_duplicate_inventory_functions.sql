-- Drop duplicate overloads of process_unified_inventory_deduction
-- Keep only the 7-parameter version with integer p_quantity_sold and text types
--
-- Current overloads in production:
-- 1. (uuid, text, integer, text, text) - 5 params, old version → DROP
-- 2. (uuid, text, integer, text, text, text, text) - 7 params, KEEP THIS ONE
-- 3. (uuid, text, numeric, text, text, text, text) - 7 params with numeric → DROP (causes ambiguity)
-- 4. (uuid, text, numeric, date, time, text) - 6 params with date/time types → DROP

-- Drop #1: Old 5-parameter version
DROP FUNCTION IF EXISTS public.process_unified_inventory_deduction(uuid, text, integer, text, text);

-- Drop #3: 7-parameter version with numeric (causes ambiguity with #2)
DROP FUNCTION IF EXISTS public.process_unified_inventory_deduction(uuid, text, numeric, text, text, text, text);

-- Drop #4: Old 6-parameter version with date/time types
DROP FUNCTION IF EXISTS public.process_unified_inventory_deduction(uuid, text, numeric, date, time, text);

-- The remaining function should be #2: the 7-parameter version with integer
-- (uuid, text, integer, text, text, text, text)

-- Also drop duplicate simulate_inventory_deduction overloads
-- This function is no longer used - client-side simulation is used instead (DRY principle)
DROP FUNCTION IF EXISTS public.simulate_inventory_deduction(uuid, text, numeric);
DROP FUNCTION IF EXISTS public.simulate_inventory_deduction(uuid, text, integer);

-- ============================================================================
-- BACKUP: Re-create the function we're keeping in case all overloads got dropped
-- This is the 7-parameter version with integer p_quantity_sold from migration 20251023175015
-- ============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- CASE 3: Density Conversion (Volume recipe units to Weight packages)
