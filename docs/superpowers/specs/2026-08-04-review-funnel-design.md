...
- A **`reviews` permission area** — the fifteenth — with `view` and `manage`
  levels, wired through all six places the existing fourteen are wired.
...
1. **`anon` gets zero grants on both new tables.** Not "RLS policies that
...
4. **Rate limiting is per page, per IP hash, per hour.** The IP is never
   stored raw: `ip_hash = encode(sha256(ip || :secret), 'hex')`. Over the
   ceiling, the function returns success and writes nothing — a silent drop,
   because an error message is a signal telling an abuser to rotate IPs.

A honeypot field (`hp`) is rendered hidden and must arrive empty. This is
cheap and catches naive form-fillers; it is not load-bearing.
...
disagrees with the page, even by the service role.

`contact_email` is only meaningful when `contact_consent` is true. The inbox
renders the address only in that case; slice 2's reply feature reads the same
flag. Storing the address without consent and hiding it in the UI would be the
wrong shape, so the edge function **discards** name and email outright when
consent is false.

Indexes: `(restaurant_id, submitted_at DESC)` for the inbox,
...
served from a public bucket is a stored-XSS vector. Public because an anonymous guest's browser has to load the image with no
credentials; the content is a logo the restaurant chose to print on a table
tent. Objects are keyed `{restaurant_id}/{review_page_id}/{uuid}.{ext}`, and
writes are restricted to `manage:reviews` holders for that `restaurant_id`.
...
Both tables: `ENABLE ROW LEVEL SECURITY`, and

```sql
REVOKE ALL ON public.review_pages, public.review_responses FROM anon;
...
`review_responses` has **no INSERT policy for `authenticated`** and no grant
for `anon`: the only writer is the edge function's service role. A restaurant
cannot manufacture its own five-star ratings through the API, which is the
whole point of the data.

The read audience matches the write audience: anyone who can see a response
row can see it in full, including the contact fields. There is no partial-row
visibility to get wrong.

---

## The `reviews` area — six files in lockstep

Adding the fifteenth area is the highest-risk part of this work, because the
area catalog is mirrored across six files and one pgTAP test asserts the SQL
and TypeScript copies agree byte-for-byte. All six change in one commit — rows
1–3 below are three separate edits inside the same new migration.

| # | File | Change |
|---|---|---|
| 1 | `supabase/migrations/…_reviews_area.sql` | `INSERT INTO area_catalog` a `reviews` row |
...
| 4 | `src/lib/permissions/types.ts:45` | `'view:reviews' \| 'manage:reviews'` in `Capability` |
| 5 | `src/lib/permissions/areas.ts` | `AreaKey` (`:37`), `AREA_DEFINITIONS` (`:146`), `AREA_CAPABILITIES` (`:254`), `AREA_LANDING_PATHS` (`:391`), `AREA_PRIORITY` (`:414`) |
| 6 | `src/lib/permissions/definitions.ts` | the two capabilities into each builtin role's `ROLE_CAPABILITIES` array |
| 7 | `src/lib/permissions/routeAreas.ts:40` | `{ path: '/reviews', area: 'reviews', minLevel: 'view' }` |
| 8 | `supabase/tests/roles_seed_test.sql` | extend the 78-row fixture and bump `plan()` |
...
(`src/App.tsx:310`). Lazy-loaded, so neither the page nor its font nor the QR
library touches the authenticated bundle.
...
theme preference of ours, and the page is paper in both.
...
- **Offline / slow:** the star tap optimistically advances the UI and reconciles;
  if the write ultimately fails, the guest is moved to the thank-you state
  anyway rather than being asked to rate twice.
...
The token payload is `{ rid, exp }` signed with `REVIEW_TOKEN_SECRET` — **a
new edge-function secret that must be set before deploy**, and the only new
piece of configuration this feature needs. `exp` is 30 minutes. Verification
reuses the shape of `verifyUnsubscribe`, including its constant-time
comparison.
...
convention established in `61e73fec`), contact with its consent state, and the
status control.
...
- The rating is announced on selection via a polite live region, because the
  visual confirmation (colour fill) is not available to everyone.
...
- Contrast is checked at AA for body copy against the paper ground, including
  the muted micro-copy, which is the tone most likely to fail.
