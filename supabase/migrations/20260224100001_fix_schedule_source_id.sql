-- Fix: include week_start_date in source_id to prevent cross-week collisions
-- The original RPC used slot_id-index which collides when generating
-- schedules for different weeks from the same template.
...
BEGIN
  ...
  LOOP
    ...
    CASE v_slot.day_of_week
      WHEN 1 THEN v_actual_date := p_week_start_date;
      WHEN 2 THEN v_actual_date := p_week_start_date + 1;
      WHEN 3 THEN v_actual_date := p_week_start_date + 2;
      WHEN 4 THEN v_actual_date := p_week_start_date + 3;
      WHEN 5 THEN v_actual_date := p_week_start_date + 4;
      WHEN 6 THEN v_actual_date := p_week_start_date + 5;
      WHEN 0 THEN v_actual_date := p_week_start_date + 6;
    ...
    FOR v_headcount_idx IN 0 .. (v_slot.headcount - 1) LOOP
      INSERT INTO public.shifts (
        ...
        COALESCE(v_slot.break_duration, 0), v_position, 'scheduled',
        'template',
      ...
      INSERT INTO public.schedule_slots (
        ...
        week_start_date, slot_index, employee_id, status
      ) VALUES (
        p_restaurant_id, v_slot.slot_id, v_shift_id,
        p_week_start_date, v_headcount_idx, NULL, 'unfilled'
      );
