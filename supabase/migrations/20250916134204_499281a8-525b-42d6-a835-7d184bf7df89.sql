-- Fix the calculate_daily_pnl function to handle missing related data properly
...
BEGIN
  ...
  -- Ensure v_net_revenue is not NULL if no row found
  ...
  -- Ensure v_food_cost is not NULL if no row found
  ...
  -- Ensure v_labor_cost is not NULL if no row found
