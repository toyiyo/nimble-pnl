...
const ToastCallback = () => {
  ...
  const [message, setMessage] = useState('Processing Toast POS connection...');
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (error) {
        setStatus('error');
        setMessage(`Toast POS connection failed: ${error}`);
        toast({
          title: "Connection Failed",
          description: "Toast POS authorization was denied or failed",
          variant: "destructive",
        });
      ...
      try {
        ...
        setStatus('success');
        setMessage('Successfully connected to Toast POS!');
        toast({
          title: "Connection Successful",
          description: "Toast POS connected! Webhooks are registered and real-time P&L updates are now active.",
        });
        ...
        setStatus('error');
        setMessage('Failed to complete Toast POS connection');
        toast({
          title: "Connection Failed",
          description: "An error occurred while connecting to Toast POS",
          variant: "destructive",
        });
  ...
  const getTitle = () => {
    switch (status) {
      case 'loading':
        return "Connecting to Toast POS...";
      case 'success':
