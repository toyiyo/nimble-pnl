-- Update the process_unified_inventory_deduction function to use enhanced unit conversions
...
BEGIN
    ...
    ELSE
        ...
        LOOP
            ...
            -- Enhanced unit conversion for rice and other products
            IF lower(v_ingredient_record.product_name) LIKE '%rice%' AND 
               v_ingredient_record.unit::text = 'cup' AND 
               COALESCE(v_ingredient_record.uom_purchase, '') = 'oz' THEN
                -- Rice-specific conversion: 1 cup = 6.3 oz
                v_purchase_unit_deduction := v_deduction_amount * 6.3;
                v_cost_per_recipe_unit := COALESCE(v_ingredient_record.cost_per_unit, 0) / 6.3;
            ELSE
                -- Default conversion or other product-specific conversions
                -- For now, use 1:1 ratio, but this should be enhanced for other products
            ...
            -- Calculate cost using enhanced conversion
            ...
            INSERT INTO inventory_transactions (
                ...
                -(v_purchase_unit_deduction * COALESCE(v_ingredient_record.cost_per_unit, 0) / 
                  CASE 
                    WHEN lower(v_ingredient_record.product_name) LIKE '%rice%' AND 
                         v_ingredient_record.unit::text = 'cup' AND 
                         COALESCE(v_ingredient_record.uom_purchase, '') = 'oz' THEN 6.3
                    ELSE 1
                  END),
                'usage',
            ...
            v_deductions := v_deductions || jsonb_build_object(
                ...
                'enhanced_conversion_used', CASE 
                    WHEN lower(v_ingredient_record.product_name) LIKE '%rice%' THEN true
                    ELSE false
                END
            );
        END LOOP;
