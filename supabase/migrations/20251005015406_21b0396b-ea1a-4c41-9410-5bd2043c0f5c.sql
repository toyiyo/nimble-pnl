-- Drop existing functions to recreate with enhanced conversion logic
...
-- Recreate process_unified_inventory_deduction with comprehensive unit conversion
...
BEGIN
    ...
    ELSE
        ...
        LOOP
            ...
            -- 2. Volume conversions to bottle/container
            ...
            -- 3. Weight conversions to lb/kg/bag
            ...
            -- Fallback: 1:1 conversion if no conversion found
...
-- Recreate simulate_inventory_deduction with same conversion logic
...
BEGIN
    ...
    FOR v_ingredient_record IN 
        SELECT ri.*, p.name as product_name, p.current_stock, p.cost_per_unit,
               p.uom_purchase, p.uom_recipe, p.conversion_factor, p.size_value, p.size_unit
        FROM recipe_ingredients ri
        JOIN products p ON ri.product_id = p.id
        WHERE ri.recipe_id = v_recipe_record.id
    LOOP
        ...
        -- SAME COMPREHENSIVE CONVERSION LOGIC AS ABOVE
        v_conversion_result := NULL;
