-- Fix reconciliation logic contradiction: allow reclassification for reconciled transactions
-- The original code blocked all categorization of reconciled transactions but suggested using
-- reclassification, creating a contradiction. This fix allows reclassification to proceed
-- for reconciled transactions while blocking initial categorization.
...
BEGIN
  ...
  -- Reclassification creates proper audit trails through journal entries
  ...
  -- Handle reclassification (now works for both reconciled and unreconciled transactions)
  IF v_is_reclassification THEN
    -- Create reclassification journal entry with transaction UUID for guaranteed uniqueness
  ...
  ELSE
    -- Initial categorization (only allowed for unreconciled transactions)
    ...
    ELSE
      -- Create new journal entry with NULL-safe entry number
      -- COALESCE ensures we never get 'BANK-NULL-...' by falling back to transaction UUID
      INSERT INTO journal_entries (
        restaurant_id,
        entry_date,
        entry_number,
  ...
  -- Update transaction - is_categorized tracks categorization state
