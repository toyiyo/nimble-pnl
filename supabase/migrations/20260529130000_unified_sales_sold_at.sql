-- Migration: unified_sales_sold_at
-- Purpose: Add nullable sold_at timestamptz column to unified_sales, populated from
--          Toast openedDate (the moment the order was served, not closed/settled).
--
-- Background:
-- unified_sales.sale_time was derived from Toast closedDate, which piles up at
-- end-of-night (~4 AM auto-settle), making staffing suggestions wildly wrong.
-- openedDate reflects when the order was placed/served, yielding a clean
-- lunch+dinner hour curve. We add sold_at as the absolute UTC instant so consumers
-- can convert to any timezone at read time. sale_time/sale_date are untouched.
--
-- Design: docs/superpowers/specs/2026-05-29-toast-sale-time-opened-date-design.md
--
-- Parts:
--   1. ADD COLUMN sold_at timestamptz (nullable, no index intentional)
--   2. Redefine sync_toast_to_unified_sales(UUID) — single-arg overload
--   3. Redefine sync_toast_to_unified_sales(UUID, DATE, DATE) — date-range overload
--   4. Bounded backfill (90 days, IS NULL guard, regex guard, tip/refund excluded)

-- =============================================================================
-- Part 1: Add sold_at column to unified_sales
-- =============================================================================
...
COMMENT ON COLUMN public.unified_sales.sold_at IS
  'Absolute instant the sale was served (UTC). Source of truth for time-of-day/hour; '
  'convert with AT TIME ZONE <restaurant_tz>. Nullable; consumers fall back to '
...
BEGIN
  ...
  -- Tips don't have closedDate in toast_payments; sold_at stays NULL for tips
  INSERT INTO public.unified_sales (
    restaurant_id, pos_system, external_order_id, external_item_id,
    item_name, quantity, unit_price, total_price,
    sale_date, sale_time, item_type, adjustment_type, raw_data, synced_at
  ...
  -- 6. REFUND entries
  -- Refunds don't have closedDate; sold_at stays NULL for refunds
  INSERT INTO public.unified_sales (
    restaurant_id, pos_system, external_order_id, external_item_id,
    item_name, quantity, unit_price, total_price,
...
-- =============================================================================
-- Part 4: Bounded backfill — recent 90 days only, IS NULL guard, tip/refund excluded
-- =============================================================================
