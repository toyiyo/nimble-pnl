-- Fix calculate_recipe_cost to use size_unit for conversions instead of uom_purchase
...
BEGIN
  ...
  LOOP
    DECLARE
      ...
      -- Use size_unit if available, otherwise fall back to uom_purchase
      actual_purchase_unit TEXT := lower(COALESCE(ingredient_record.size_unit, ingredient_record.uom_purchase, ''));
    BEGIN
      -- Handle direct unit matches first
      IF recipe_unit_lower = actual_purchase_unit THEN
        ...
        cost_impact := ingredient_record.quantity * cost_per_purchase_unit;
...
-- Recalculate the maple syrup recipe cost
UPDATE recipes 
SET estimated_cost = calculate_recipe_cost(id)
WHERE id = '9850d3ce-709d-4444-9bb0-b18ac5183fa2';