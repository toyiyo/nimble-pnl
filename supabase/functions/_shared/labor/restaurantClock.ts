 ...
 * calendar-day half lives in `./dateOnly.ts`. Each rejects the other's
 * input rather than silently producing a plausible wrong answer.
 */

// Type-only declaration for `import.meta.env`. Vite and Vitest fill it in the
// browser and in tests (`vi.stubEnv`). Deno has no `env` on `ImportMeta`, so
// the Deno type check needs this. At runtime in Deno the value is undefined,
// and `reject()` reads it with `?.`. The members repeat the Vite declarations
// exactly, so the two declarations merge without a conflict.
declare global {
