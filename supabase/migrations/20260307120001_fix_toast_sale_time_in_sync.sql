-- Fix sync_toast_to_unified_sales to derive sale_time from closedDate
-- in the restaurant's local timezone, instead of using toast_orders.order_time
-- which is always NULL (by design — businessDate has no time component).
--
-- This replaces `too.order_time` with a timezone-aware extraction from raw_json.
...
COMMENT ON FUNCTION sync_toast_to_unified_sales(UUID) IS
  'Syncs ALL Toast orders to unified_sales. Derives sale_time from closedDate in restaurant timezone. Skips per-row triggers via GUC flag during bulk ops, then batch-categorizes and batch-aggregates after sync.';

-- =============================================================================
-- Date-range overload: same fix applied
-- =============================================================================
...
COMMENT ON FUNCTION sync_toast_to_unified_sales(UUID, DATE, DATE) IS
  'Syncs Toast orders within date range to unified_sales. Derives sale_time from closedDate in restaurant timezone. Skips per-row triggers via GUC flag during bulk ops, then batch-categorizes and batch-aggregates after sync.';
