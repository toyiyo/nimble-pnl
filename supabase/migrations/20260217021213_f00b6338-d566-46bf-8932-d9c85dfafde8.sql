...
BEGIN
  ...
  LOOP
    ...
    IF v_restaurant_id IS NULL OR v_week_end IS NULL THEN
      ...
      VALUES (
        CASE WHEN v_restaurant_id IS NOT NULL THEN v_restaurant_id::uuid ELSE NULL END,
        CASE WHEN v_week_end IS NOT NULL THEN v_week_end::date ELSE NULL END,
        'dead_lettered',
        v_msg.read_ct,
        'Invalid payload: missing restaurant_id or week_end'
      );
      CONTINUE;
    END IF;
