-- Allow re-splitting already categorized transactions
-- Users should be able to split a transaction even if it's already categorized
...
BEGIN
  ...
  -- If transaction is already split, delete existing splits
  ...
  IF v_existing_journal_entry IS NOT NULL THEN
    -- Delete existing journal entry lines and reuse the entry
    ...
    SET
      entry_number = 'SPLIT-' || v_transaction.stripe_transaction_id || '-' || TO_CHAR(now(), 'YYYYMMDD-HH24MISS-US'),
      description = 'Split transaction: ' || v_transaction.description,
      updated_at = now()
    WHERE id = v_journal_entry_id;
  ELSE
    -- Create new journal entry for split transaction
    INSERT INTO journal_entries (
      ...
      'SPLIT-' || v_transaction.stripe_transaction_id || '-' || TO_CHAR(now(), 'YYYYMMDD-HH24MISS-US'),
