...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (authHeader) {
      ...
      const { data: userRestaurant, error: restaurantError } = await supabase.from('user_restaurants').select('role').eq('user_id', user.id).eq('restaurant_id', restaurantId).single();
    ...
    console.log('Shift4 sync started:', {
      restaurantId,
      action,
      dateRange,
      userId
    ...
    const { data: connection, error: connError } = await supabase.from('shift4_connections').select('*').eq('restaurant_id', restaurantId).single();
    ...
    try {
      ...
      let lighthouseToken = null;
      const now = new Date();
      const expiresAt = connection.lighthouse_token_expires_at ? new Date(connection.lighthouse_token_expires_at) : null;
      ...
      for (const ticket of tickets){
        ...
        if (ticket.status && typeof ticket.status === 'string' && ticket.status.toLowerCase().includes('void')) {
          stats.ticketsVoided++;
          console.log(`[Lighthouse Sync] Skipping voided ticket`, {
            ticketStatus: ticket.status,
            orderNumber: ticket.orderNumber || ticket.ticket || ticket.ticketNumber
          });
          continue;
        }
        ...
        const { data: existingRows, error: existingErr } = await supabase.from('unified_sales').select('external_item_id').eq('restaurant_id', restaurantId).eq('pos_system', 'lighthouse').eq('external_order_id', chargeId);
        if (existingErr) {
        ...
        if (duplicatesThisTicket > 0) {
          ...
          console.warn(`[Lighthouse Sync] Skipped duplicates for ticket`, {
            orderNumber,
            chargeId,
            duplicatesThisTicket
          });
