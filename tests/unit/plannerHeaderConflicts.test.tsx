 ...
 *
 * Invariants:
 * 1. conflictedShiftCount = 1 renders "1 conflict"; above one renders "N conflicts".
 * 2. conflictedShiftCount = 0 (or absent) renders no pill.
...
describe('PlannerHeader — conflict rollup', () => {
  it('renders the singular pill for one conflict', () => {
    render(<PlannerHeader {...baseProps} conflictedShiftCount={1} />);
    expect(screen.getByText('1 conflict')).toBeTruthy();
  });

  it('renders the plural pill above one', () => {
    render(<PlannerHeader {...baseProps} conflictedShiftCount={3} />);
    expect(screen.getByText('3 conflicts')).toBeTruthy();
  });

  it('renders no pill at zero and when absent', () => {
    const { rerender } = render(<PlannerHeader {...baseProps} conflictedShiftCount={0} />);
  ...
  it('renders the muted note instead of the pill when conflicts are unavailable', () => {
    render(<PlannerHeader {...baseProps} conflictedShiftCount={3} conflictsUnavailable />);
