...
export interface EmployeePayroll {
  ...
  doubleTimePay: number;         // Double-time pay in cents
  dailyOvertimeHours: number;    // Daily OT hours (subset of overtimeHours)
  weeklyOvertimeHours: number;   // Weekly OT hours (subset of overtimeHours)
...
interface ShiftParsingState {
  ...
  currentClockIn: LaborTimePunch | null;
 ...
 */
function deduplicatePunches(punches: LaborTimePunch[]): LaborTimePunch[] {
  if (punches.length <= 1) return punches;

  const result: LaborTimePunch[] = [];
  let i = 0;
...
export function calculateEmployeePay(
  ...
  const grossPay = regularPay + overtimePay + doubleTimePay + salaryPay + contractorPay + dailyRatePay + manualPaymentsTotal;
...
export function calculatePayrollPeriod(
  ...
  punchesPerEmployee: Map<string, LaborTimePunch[]>,
