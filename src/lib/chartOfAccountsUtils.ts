...
export const createDefaultChartOfAccounts = async (supabase: SupabaseClient, restaurantId: string) => {
  // First pass: insert accounts without parent references
  const accountsWithoutParents = DEFAULT_ACCOUNTS.filter(acc => !('parent_code' in acc));
  const accountsWithParents = DEFAULT_ACCOUNTS.filter(acc => 'parent_code' in acc);
