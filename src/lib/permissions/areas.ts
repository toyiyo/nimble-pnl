...
import { PAGE_AREA_ROWS } from './areaData';
 ...
 * `PAGE_AREA_ROWS` (areaData.ts — split out there so its structurally
 * repetitive rows don't score as duplication against this file's actual
 * logic; see that file's header). Source of truth for `AREA_DEFINITIONS`
 * (joined against `navigationGroups` for label/icon/group/order),
 * `AREA_LANDING_PATHS` and `AREA_PRIORITY` below — none of those three is
