import type { LaborTimePunch } from './types.ts';
...
function lastPunch<P extends LaborTimePunch>(sorted: P[]): P | undefined {
 ...
 */
export function appendOpenShiftClockOuts<P extends LaborTimePunch>(
  punches: P[],
  now: Date,
  ...
  // A synthetic punch has only the LaborTimePunch fields, as before the move.
  // The cast to P is type-only.
  const synthetic: P[] = [];
  for (const [employeeId, list] of byEmployee) {
    ...
    if (last.punch_type === 'break_start') {
      synthetic.push({ ...base, id: `synthetic-${employeeId}-break_end`, punch_type: 'break_end' } as P);
    }
    synthetic.push({ ...base, id: `synthetic-${employeeId}-clock_out`, punch_type: 'clock_out' } as P);
  }
