...
### [2026-05-10] `new Date("YYYY-MM-DD")` parses as UTC midnight — Postgres `DATE` columns render off-by-one in non-UTC browsers
- **Mistake:** Tristen Liu reported her May 29 graduation-day time-off request showing as May 28 in the UI while the manager-approval email correctly said May 29. The Postgres `time_off_requests.start_date` (a `DATE` column) stored `2026-05-29` correctly. The display path used `format(new Date(request.start_date), 'MMM d, yyyy')`, but per the ECMAScript spec a date-only ISO string is parsed as UTC midnight — so in any TZ behind UTC (US Central, Pacific, Mountain, Eastern), `format()` then renders the previous day. The email worked because Supabase Edge Functions (Deno) run in UTC, masking the bug for any rendering that happened server-side. The dialog write path also round-tripped through `fromZonedTime(date, restaurantTimezone).toISOString().substring(0,10)` for a value that is already a calendar day — pulling a TZ dependency into a column that has no TZ. PR #489.
- **Correction:** Added `src/lib/dateOnly.ts` with three pure helpers: `parseDateOnly` (regex-validates `YYYY-MM-DD`, constructs `new Date(year, month-1, day)` to anchor at LOCAL midnight); `toDateOnlyString` (uses `getFullYear/getMonth/getDate` — local fields, not UTC); `formatDateOnly` (composes the two for date-fns rendering). Swapped four user-visible time-off render/persist sites: `TimeOffList`, `EmployeePortal`, and both branches of `TimeOffRequestDialog` (calendar prefill + form submit). Removed the dialog's `RestaurantContext` + `date-fns-tz` dependency for date math. Added 14 TZ-independent vitest cases (asserting on `getFullYear/getMonth/getDate`, not ISO strings) including a "Tristen Liu off-by-one" regression and a "documents the trap" case showing why `new Date("YYYY-MM-DD").toISOString()` is `T00:00:00.000Z` regardless of host TZ.
- **Rule:** A Postgres `DATE` column is a calendar day with no timezone. Treat it as a `YYYY-MM-DD` *string* end-to-end — never round-trip it through `new Date(string)` + `format()`, and never push it through `fromZonedTime` with the user's IANA TZ. For tests, assert wall-clock fields (`getFullYear/getMonth/getDate`) so they hold in any host TZ — `toISOString()` and `toLocaleDateString()` are TZ-dependent and useless as fixtures. The companion bug class — `T00:00:00` / `T23:59:59` literal-string suffix appended to the column value — is a *valid* sidestep (no TZ offset → local time per spec) and exists in `src/lib/shiftValidator.ts` for conflict-overlap math; don't "fix" it.
