-- Fix deduplication to use external_order_id for uniqueness
-- This prevents false duplicates when multiple sales of same item occur on same day

-- Drop and recreate the check function with better uniqueness logic
DROP FUNCTION IF EXISTS public.check_sale_already_processed(uuid, text, integer, text);

CREATE OR REPLACE FUNCTION public.check_sale_already_processed(
...
DECLARE
    v_reference_id text;
BEGIN
    -- Create reference ID with external_order_id for true uniqueness
    -- If no external_order_id provided, fall back to old format for backwards compatibility
...
-- Update process_unified_inventory_deduction to use external_order_id in reference_id
-- This is the main function that processes inventory deductions
...
-- Create new function to bulk process historical sales
...
BEGIN
    -- Process all sales in the date range
    FOR v_sale IN 
        SELECT 
            item_name,
            quantity,
