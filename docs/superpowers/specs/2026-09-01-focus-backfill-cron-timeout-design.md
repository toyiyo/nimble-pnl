...
callers persist the cursor with `.eq('sync_cursor', readCursor)`
(see `supabase/functions/_shared/focusBackfillBatch.ts`). The error is
cosmetic, but it repeats on every 5-minute tick during a backfill.
...
margin, and stays below the edge function wall-clock limit (150 s).
...
and its workers claim connections atomically with `SKIP LOCKED`, so a
duplicate dispatch there is safe.
...
- If a run exceeds 120 s, the duplicate-worker race returns. The observed
  ceiling is 90 s; the batch loop in `focusBackfillBatch.ts` bounds each run.
