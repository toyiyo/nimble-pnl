-- Restore the capability guard on bulk_categorize_bank_transactions.
--
-- History: migration 20260820100000 (PR #770) added Guard 1b, a
-- user_has_capability('edit:transactions') check, to this RPC. Migration
-- 20260820210200 (PR #772) changed the entry-day derivation in the same
-- RPC. It sourced its CREATE OR REPLACE from 20260819231210, the pre-guard
-- body. The two branches merged with no text conflict, because each change
-- lives in its own migration file. On a fresh database the migrations run
-- in filename order, so 20260820210200 wins and the guard is gone.
--
-- Fix: replace the function one more time. The body below is the body from
-- 20260820210200 (restaurant-local entry day) plus the Guard 1b block from
-- 20260820100000. The Guard 1b block is the only difference from
-- 20260820210200.
--
-- See docs/superpowers/specs/2026-08-20-bulk-categorize-capability-guard-design.md
-- section 7 (amendment of 2026-08-21).
